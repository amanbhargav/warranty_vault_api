...
const api = axios.create({
  ...
  // CRITICAL: Send cookies with every request for cookie-based auth
  withCredentials: true,
  // Disable XSRF since we're using custom cookie auth
...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      // Don't redirect for login/signup requests - let the form handle errors
      const isAuthRequest = error.config?.url?.includes('/auth/login') ||
                           error.config?.url?.includes('/auth/signup');
      
      if (!isAuthRequest) {
        // Clear local storage on auth failure for other requests
